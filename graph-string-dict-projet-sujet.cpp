...
// Vous devez représenter un graphe sous la forme de liste d'matrice, constitué de sommets nommés
// et d'arêtes portant une valeur numérique (double) entre ces sommets.

// Dans un graphe sous la forme de liste d'matrice:
...
//    6) vos devez implémenter une méthode de Graph qui construit la matrice d'matrice du graphe
//       vous stockez cette matrice comme attribut du graphe
//       utilisez votre class Matrix ou codez-en une

//    7) appliquez à cette matrice d'matrice l'Algorithme de Floyd-Warshall
...
public:
    ...
    friend class Graph;
    friend class Vertex; /* votre code ici */
};
class Vertex
{
    private:
...
public:
    ...
    friend class Graph;/* votre code ici */
};
...
public:
    ...
    void add_edge(const std::string &begin,
                  const std::string &end,
                  double value){
    // Si le sommet "begin" n'existe pas, on le crée
    if (indexMap.find(begin) == indexMap.end())
    {
        Vertex* nouveau = new Vertex(begin);
        vertices.push_back(nouveau);
        indexMap[begin] = vertices.size() - 1;
    }

    // Si le sommet "end" n'existe pas, on le crée
    if (indexMap.find(end) == indexMap.end())
    {
        Vertex* nouveau = new Vertex(end);
        vertices.push_back(nouveau);
        indexMap[end] = vertices.size() - 1;
    }

    //On récupère leurs indices
    int indice_depart = indexMap[begin];
    int indice_arrivee = indexMap[end];

    //On crée l'arête
    Edge* nouvelle_arete = new Edge(vertices[indice_arrivee], value);

    // On l'ajoute à la liste des arêtes du sommet de départ
    vertices[indice_depart]->edges.push_back(nouvelle_arete);
}
    ...
    void dfs()
{
    ...
    std::function<void(int)> dfs_visit = [&](int index)
    {
        visited.insert(index);
        std::cout << vertices[index]->name << " ";

        for (Edge* e : vertices[index]->edges)
        {
            int next = indexMap[e->destination->name];
...
{
    int nombre_sommets = vertices.size();

    // On redimensionne la matrice
    matrice.resize(nombre_sommets);

    for (int i = 0; i < nombre_sommets; i++)
    {
        matrice[i].resize(nombre_sommets);
    }
    ...
    for (int i = 0; i < nombre_sommets; i++)
    {
        ...
        {
            if (i == j)
                matrice[i][j] = 0;      // distance vers soi-même
            else
                matrice[i][j] = -1;     // pas d'arête (valeur simple pour débutant)
        }
    }

    // On remplit avec les vraies arêtes
    for (int i = 0; i < nombre_sommets; i++)
    {
        for (Edge* arete : vertices[i]->edges)
        {
            int j = indexMap[arete->destination->name];
            matrice[i][j] = arete->weight;
        }
    }
}
...
int main()
{
    ...
    // LECTURE DU GRAPHE
    Graph graph = read_triplet("graph0.gr.save");
    graph.dfs();
