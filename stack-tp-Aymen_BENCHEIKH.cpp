...
{
    if (is_full(stack, size, nb))
        throw std::overflow_error(std::string("erreur: la pile est pleine !"));  
...
int top(int stack[], int size, int nb /* les arguments ici */) // (cette fonction ne dépile pas)
...
// elle lance une exception si problème
{
    if (is_empty(stack, size, nb)){

        throw std::overflow_error(std::string("erreur: la pile est vide !"));  
...
int main()
{
    ...
    {
        std::cerr << "Exception attrapée: "<< e.what() << std::endl;    // ce catch ramasse toutes les exceptions...
    }
}